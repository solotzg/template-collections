...
struct BridgeBuilding {
    ...
    int go(VI & a, VI & b, int K) {
        int n = a.size()+1;
        static int sa[N], sb[N], mx[N][N], mxl[N][N], mxr[N][N], ma[N][N], tpv[N*2], dp[N][N];
        ...
        }
        {
            ...
            while (bg < ed) {
                ...
                mst(dp, inf);
                rep(i,0,n) {
                    dp[i][1] = (tp=max(sa[i], sb[i]))>mid || sa[i]+sb[i] > mid?inf:tp;
                }
                rp(i,n) repd(k,1,K-1) if (dp[i][k] != inf) {
                    rep(o, i+1, n) if (ma[i][o] <= mid) {
                        if ((tp=dp[i][k]+mxl[i][o]) <= mid) {
                            dp[o][k+1] = min(dp[o][1+k], max(mxr[i][o], dp[i][k]+mx[i][o]));
                        }
                    }
                }
                int ans = inf;
                rp(i, n) if (dp[i][K] != inf && (sa[n-1]-sa[i] + sb[n-1]-sb[i])<=mid && (tp = dp[i][K]+max(sa[n-1]-sa[i], sb[n-1]-sb[i])) <= mid) {
                    ans = min(ans, tp);
                }
                if (ans <= mid) ed = mid;
                else bg = mid+1;
...
{2,4,10,2,2,22,30,7,28,14,17,6,32,41,1,3,16,20,40,39,6,4,18,3,3,7,
