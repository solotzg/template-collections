...
struct Matrix {
  ll d[MAXM][MAXM];
  int r, c;
  void one() { rp(i, r) d[i][i] = 1; }
  Matrix(int rr, int cc) : r(rr), c(cc) { mst(d, 0); }
  Matrix() {
    mst(d, 0);
    r = c = -1;
  }
  Matrix operator*(const Matrix &a) const {
    ...
    rp(i, res.r) {
      rp(j, res.c) {
        rp(k, c) { dd[i][j] = (dd[i][j] + d[i][k] * a.d[k][j] % mod) % mod; }
      }
    }
    return res;
  }
  Matrix operator+(const Matrix &a) const {
    Matrix res(r, c);
    rp(i, r) rp(j, c) { res.d[i][j] = (d[i][j] + a.d[i][j]) % mod; }
    return res;
  }
  void show() {
    rp(i, r) {
      rp(j, c) { printf("%lld ", d[i][j]); }
      puts("");
    }
    puts("");
  }
...
struct CountBinarySequences {
  ...
  Matrix even_matrix(int m) {
    ...
    Matrix R(k + 1, k + 1);
    rp(i, k + 1) rp(j, k + 1) { R.d[i][j] = X.d[i][j]; }
    return R;
  }
  Matrix odd_matrix(int m) {
    Matrix X = pow(base_matrix, m);
    Matrix R(k + 1, k + 1);
    rp(i, k + 1) rp(j, k + 1) { R.d[i][j] = X.d[i][j + k + 1]; }
    return R;
  }
};
int CountBinarySequences::countSequences(int _n, int _k, vector<int> _L,
                                         vector<int> _R) {
  n = _n;
  k = _k;
  L = _L;
