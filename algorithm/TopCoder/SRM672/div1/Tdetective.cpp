  ...
  int reveal(vector<string> s) {
    ...
    urp(i, 9, 0) {
      rp(a, n) rp(b, n) if (p[a][b] == i && find(a) != find(b))
          rp(c, n) if (find(a) == find(c)) r[c][b] = min(r[c][b], o[find(c)]);
      rp(a, n) rp(b, n) if (p[a][b] == i && find(a) != find(b)) o[find(a)] +=
          o[find(b)],
          f[find(b)] = find(a);
    }
    rp(k, n) rp(i, n) rp(j, n) if (i != j && j != k && i != k) r[i][j] =
        min(r[i][j], r[i][k] + r[k][j]);
    rp(i, n) ans += i * r[0][i];
    return ans;
  }
};
