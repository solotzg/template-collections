...
struct Tdetectived2 {
  int reveal(vector<string> s) {
    ...
    rep(task, 1, M) if (num[task] != inf) {
      max_levels = -1;
      mst(levels, -1);
      rp(i, n) if ((1 << i) & task) rp(j, n) levels[j] =
          max(levels[j], S[i][j]);
...
int main() {
  ...
  assert(a->reveal({"0103122326302335", "1011563543411505", "0103661036520111",
                    "3130226506025653", "1562015121602260", "2662105511354602",
                    "2316550151110446", "3505151015062410", "2430215100055423",
                    "6366111500421250", "3450631004015304", "0122051652101164",
                    "2105240251510402", "3516264442314046", "3015604125060401",
                    "5513026030442610"}) == 640);
  return 0;
}
