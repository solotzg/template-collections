  ...
  repd(n, 1, N - 1) {
    ...
    repd(na, 0, n) repd(nb, 0, n) {
      ...
      repd(k, 0, mk) if (dp[cur][na][nb][k]) {
        ...
        dp[nxt][na + 1][nb][k] = 1;
        res[nxt][na + 1][nb][k] = s + 'A';
        dp[nxt][na][nb + 1][k + na] = 1;
        res[nxt][na][nb + 1][k + na] = s + 'B';
        dp[nxt][na][nb][k + na + nb] = 1;
        res[nxt][na][nb][k + na + nb] = s + 'C';
      }
    }
    cur = nxt;
    nxt = 1 - nxt;
...
int main() {
  ABC a;
  cout << a.createString(30, 345) << endl;
  return 0;
}
