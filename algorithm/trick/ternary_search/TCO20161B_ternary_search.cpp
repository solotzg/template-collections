...
struct SettingShield {
  long long getOptimalCost(int n, int h, int t, vector<int> val0, vector<int> a,
                           vector<int> b, vector<int> m) {
    VI protection(n);
    protection[0] = val0[0];
    rep(i, 1, n) protection[i] =
        (1ll * a[0] * protection[i - 1] % m[0] + b[0]) % m[0];
    VI mx_right(n + 2, -1);
    ...
    rep(i, 1, n) if (mx_right[i - 1] >= i) mx_right[i] =
        max(mx_right[i], mx_right[i - 1]);
    int bg = 0, ed = *max_element(vtr(protection));
    rp(i, n) if (mx_right[i] == -1) { bg = max(bg, protection[i]); }
    auto cal = [&](int p) -> ll {
      ll cost = 1ll * p * t;
    ...
    ll res = 2e18;
    repd(x, bg, ed) { res = min(res, cal(x)); }
    return res;
  }
};
