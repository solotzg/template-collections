...
struct BearFair2 {
    ...
    int go(int cn, int b, VI & up, VI & qu) {
        cs[0][0] = cs[1][0] = 0;
        rep(i,1,M) cs[0][i]=cs[0][i-1]+(i%3==0), cs[1][i]=cs[1][i-1]+(i%3==1);
        vector<PII> p;
        rp(i, up.size()) {
            p.pb(mp(up[i], qu[i]));
        }
        ...
        rep(i,1,n) {
            ...
            if (b <0 || a < b) return 0;
            int t0 = cs[0][p[i].fi]-cs[0][p[i-1].fi], t1 = cs[1][p[i].fi]-cs[1][p[i-1].fi], t2 = a-t0-t1;
            repd(c0, 0, t0) repd(c1, 0, t1) if (c0+c1 <= b && b-c0-c1 <= t2) {
                repd(p0, 0, cn) repd(p1,0,cn) if (dp[i-1][p0][p1]) dp[i][p0+c0][p1+c1] = 1;
            }
        }
