...
struct TheGridDivOne {
    ...
    int find(vector <int> x, vector <int> y, int k) {
        ...
        rp(i, n) rp(j,m) mmp[i][j] = mp(mp(tx[i], ty[j]), blk.find(mp(tx[i], ty[j]))!=blk.end() );
        int ox = lower_bound(tx.begin(), tx.begin()+n , 0) - tx.begin(), oy = lower_bound(ty.begin(), ty.begin()+m, 0) - ty.begin();
        ...
        while (!q.empty()) {
            ...
            int v = nd.v;
            q.pop();
            if (nd.v != dis[px][py]) continue;
            res = max(res, omp.fi.fi);
//			printf("cur : %d %d %d\n", omp.fi.fi, omp.fi.se, v);
            int xx, yy;
            rp(i, 4) {
                xx = px+dx[i], yy = py+dy[i];
                if (_0(xx>=0 && xx < n && yy >=0 && yy < m )) continue;
                auto & fmp = mmp[xx][yy];
                if (fmp.se == 1) continue;
                if (fmp.fi.fi == INF) {
                    ...
                    if (dis[xx][yy] > ad) {
                        if (ad > k) {
                            res = max(res, omp.fi.fi+min( k-v, diffx)*(fmp.fi.fi - omp.fi.fi < 0 ?-1:1) );
                            continue;
                        }
                        dis[xx][yy] = ad;
                        q.push({ ad, xx, yy });
...
int main() {
    ...
    {-1000000000, 1000000000, -1000000000, 1000000000, -2, -1, 0, 1, 2, 3}, 1000000000
    )<<endl;
}
